    ...
    <div className="px-4 md:px-6">
      ...
      <div className="mb-12 grid grid-cols-1 items-stretch gap-10 lg:grid-cols-2">
        ...
        >
          <SectionCard className="relative overflow-hidden p-0">
            <div className="relative aspect-4/5 w-full bg-[#1a1a1a]">
              ...
              <div className="absolute right-4 top-4 flex items-center gap-1.5 rounded-full border border-[#262626] bg-black/60 px-3 py-1.5 backdrop-blur-sm">
                <span className="relative flex size-2">
                  <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#0095f6] opacity-75" />
                  <span className="relative inline-flex size-2 rounded-full bg-[#0095f6]" />
                </span>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <p className="text-sm font-medium text-[#0095f6]">
                  {profile.jobTitle}
                </p>
        ...
        <div className="flex flex-col justify-center gap-6">
          <div className="overflow-hidden rounded-xl border border-[#262626] bg-[#0d0d0d] shadow-2xl">
            <div className="flex items-center gap-2 border-b border-[#262626] bg-[#161616] px-4 py-2.5">
              <span className="size-3 rounded-full bg-[#ff5f56]" />
              <span className="size-3 rounded-full bg-[#ffbd2e]" />
              <span className="size-3 rounded-full bg-[#27c93f]" />
              <span className="ml-2 flex items-center gap-1.5 text-xs text-[#737373]">
                <Terminal className="size-3.5" />
                whoami.sh
              </span>
            </div>
            <div className="min-h-55 px-5 py-5 font-mono text-sm leading-relaxed text-[#a8a8a8]">
              {renderedLines.map((line, i) => (
                <p key={i} className="mb-3">
                  <span className="mr-2 text-[#0095f6]">$</span>
                  {line}
                </p>
              ))}
              {!done && (
                <p className="mb-3">
                  <span className="mr-2 text-[#0095f6]">$</span>
                  {currentText}
                  <span className="ml-0.5 inline-block h-4 w-2 animate-pulse bg-[#0095f6] align-middle" />
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              asChild
              className="border-0 bg-[#0095f6] text-white hover:bg-[#1877f2]"
            >
              <a
      ...
      <div className="mb-12 flex flex-col gap-6">
        <SectionCard delay={0.05}>
          <h3 className="mb-5 flex items-center gap-2 text-xl font-bold">
            <Info className="size-5 text-[#0095f6]" />
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {personalRows.map((row) => {
              ...
              return (
                <div
                  key={row.id}
                  className="flex items-start gap-3 rounded-lg border border-[#262626] bg-black/30 px-3.5 py-3 transition-colors hover:border-[#0095f6]/40"
                >
                  <Icon className="mt-0.5 size-4 shrink-0 text-[#0095f6]" />
                  <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                    <dt className="shrink-0 text-xs text-[#737373]">
                      {row.label}
                    </dt>
        ...
        <SectionCard delay={0.1}>
          <h3 className="mb-6 flex items-center gap-2 text-xl font-bold">
            <Heart className="size-5 text-[#0095f6]" />
                ...
                >
                  <div className="relative size-16 shrink-0">
                    <div
                      ...
                      style={{
                        background:
                          "conic-gradient(from 0deg, #0095f6, #1877f2 35%, transparent 70%)",
                        animationDuration: `${5 + index}s`,
                        animationDirection: reverse ? "reverse" : "normal",
                      }}
                    />
                    <div className="absolute inset-0.75 flex items-center justify-center rounded-full bg-[#0d0d0d] transition-colors group-hover:bg-[#0095f6]/10">
                      <Icon className="size-6 shrink-0 text-[#0095f6] transition-transform group-hover:scale-110" />
                    </div>
                  </div>
      ...
      <section>
        ...
        <div className="relative mx-auto max-w-2xl">
          <div className="absolute left-2.25 top-2 h-[calc(100%-1rem)] w-px bg-linear-to-b from-[#0095f6] to-[#1877f2] md:left-1/2 md:-translate-x-1/2" />
          <div className="space-y-8">
            {journey.map((item, index) => (
              <div key={item.year} className="relative pl-8 md:pl-0">
                <div className="absolute left-0 top-1.5 flex size-5 items-center justify-center rounded-full border-4 border-black bg-[#0095f6] md:left-1/2 md:-translate-x-1/2">
                ...
                >
                  <SectionCard delay={index * 0.08}>
                    <div
                      className={`mb-2 flex items-center gap-2 font-mono text-xs text-[#737373] ${
                        index % 2 === 0 ? "md:justify-end" : ""
                      }`}
                    >
                      <span className="rounded bg-[#161616] px-1.5 py-0.5 text-[#0095f6]">
                        {item.hash}
                      </span>
                      <span>{item.year}</span>
                    </div>
                    ...
                    <p className="text-sm text-[#a8a8a8]">{item.description}</p>
                  </SectionCard>
                </div>
              </div>
            ))}
